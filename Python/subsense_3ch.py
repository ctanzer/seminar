...
class PBAS(threading.Thread):
    ...
    # INITIALIZATION ===============================================================
    def __init__(self, T_r=0.003, N_grid=16, nmbr_min_lbsp=12, N_color=50, nmbr_min_color=2, R_color=30,R_lbsp=3, T_lower=2, T_upper=256, alpha=0.03, v_incr=1, v_decr=0.1,downsample = 1):
    ...
    def run(self):
        ...
        self.large_background = cv2.resize(self.background,None,fx=1/self.downsample, fy=1/self.downsample, interpolation = cv2.INTER_CUBIC)
...
if __name__ == '__main__':
    ...
    downsample = 0.7    # Create an instance per channel
    pbas_r = PBAS(T_r, N_grid, nmbr_min_lbsp, N_color, nmbr_min_color, R_color,R_lbsp, T_lower, T_upper, alpha, v_incr, v_decr, downsample)
    pbas_g = PBAS(T_r, N_grid, nmbr_min_lbsp, N_color, nmbr_min_color, R_color,R_lbsp, T_lower, T_upper, alpha, v_incr, v_decr, downsample)
    pbas_b = PBAS(T_r, N_grid, nmbr_min_lbsp, N_color, nmbr_min_color, R_color,R_lbsp, T_lower, T_upper, alpha, v_incr, v_decr, downsample)
    ...
    while True:
        ...
        background = np.logical_or(np.logical_or(pbas_r.background, pbas_g.background), pbas_b.background) * 1.
        large_background = np.logical_or(np.logical_or(pbas_r.large_background, pbas_g.large_background), pbas_b.large_background) * 1.
